...
const SvoemmeHoldData = ref([]);
const SvoemmeHoldCategoriesData = ref([]);

// fetcher alle svømmehold
const baseEndpoint = `https://runepjetursson.com/mmd4/skansen/wp-json/wp/v2/
svommehold?acf_format=standard&per_page=100`;
...
// Fetcher alle categorier under svømmehold, ved at benytte parent category id'et
const baseEndpointCategories = `https://runepjetursson.com/mmd4/skansen/wp-json/wp/v2/categories?acf_format=standard&parent=14`;

function fetchSubcategories() {
  fetch(baseEndpointCategories)
    .then((res) => res.json())
    .then((categories) => {
      SvoemmeHoldCategoriesData.value = categories;
    })
    .catch((err) => console.error(err));
}
...
const categorizedSvoemmeHoldData = computed(() => {
  const categorizedData = {};

  // Kører gennem dataen for hvert svømmehold og kategoriserer dem efter kategori
  SvoemmeHoldData.value.forEach((hold) => {
    const categoryId = hold.categories[0];
    const categoryName = getCategoryName(categoryId);
    if (!categorizedData[categoryName]) {
      categorizedData[categoryName] = [];
    }
    // Push the hold into its category
  ...
  // Sort each category alphabetically and sort svømme holds within each category
...
function getCategoryName(categoryId) {
  const category = SvoemmeHoldCategoriesData.value.find(
...
const categoryImages = computed(() => {
  const images = {};
  for (const category in categorizedSvoemmeHoldData.value) {
    const firstPost = categorizedSvoemmeHoldData.value[category][0];
...
// Function der scroller til den kategori der er klikket på
...
// Fjerner eventlitener hvis overlayet ikke er unmounted
onUnmounted(() => {
...
<template>
  <!-- Valg af kategori -->
  <section class="categoryCont flex align" role="list">
    <!-- Laver en article for hvert kategori, med tilhørende billede hentet fra holdene -->
    <article
      v-for="(hold, category) in categorizedSvoemmeHoldData"
      :key="category"
      @click="scrollToCategory(category)"
      role="listitem"
      tabindex="0"
      aria-label="Klik for at se hold i kategorien"
    >
      <img
  ...
  <!-- Sectionen for hvert hold. Div med v-for bruges til at sortere efter kategori -->
  <!-- id=category bruges til scroll functionen -->
  <div
    v-for="(holdbeskrivelse, category) in categorizedSvoemmeHoldData"
    :key="category"
    class="pageCont"
    :id="category"
    role="region"
    aria-label="Hold i kategorien"
  >
    <section>
      <div class="categoryNameCont">
        <h2 v-html="category"></h2>
      </div>
      <div class="flex wrap flexContainer" role="list">
        <div
          ...
          role="listitem"
          tabindex="0"
          aria-label="Klik for at se holdet"
        >
          <div class="nameCont">
