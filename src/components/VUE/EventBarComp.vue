...
<template>
  <section class="blue-sunrise flex" aria-live="polite" role="region" aria-label="Event Calendar">
    ...
    <div>
      ...
      <hr />
      <!-- If there is no time for today's event and thus no event, it should display the date and name of the next event -->
      <p>{{ tidspunkt }}</p>
      <div v-if="tidspunkt == ''">
        <p class="nextEvent">
          Næste event:
    ...
    <!-- Use a slot for the button to insert the necessary values -->
    <slot />
  </section>
</template>
